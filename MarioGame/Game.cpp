...
void Game::initAudio()
{
	...
	//Main theme
	this->game_audio_manager->addSound("Main Theme", "Sounds/Game/Ground Theme.wav", true);
	this->game_audio_manager->playSound("Main Theme");
}

void Game::initText()
{
	this->main_font.loadFromFile("Fonts/SuperMario85.ttf");
...
{
	sf::Vector2f newPos = sf::Vector2f(MathUtils::clamp(400.f + mario->getPosition().x - 400.f, 0.f + this->window->getSize().x / 2.f, 1000.f), 300.f);
...
void Game::render()
{
	this->window->clear(sf::Color(92, 148, 252, 255));
